...
export const AppShell: React.FC<AppShellProps> = ({
  ...
  const { bottomSheet, closeBottomSheet, openBottomSheet, setPageChrome, clearPageChrome } = useUIStore();
  const navigate = useNavigate();

  React.useEffect(() => {
    setPageChrome(title ?? null, subtitle ?? null);
    return () => clearPageChrome();
  }, [title, subtitle, setPageChrome, clearPageChrome]);

  // Flush panes (inbox/brain): title lives in TopBar only. Mobile: TopBar owns the title.
  const hasContentHeader = flush
    ? !!actions
    : !!(subtitle || actions || (!isMobile && title));
      ...
      <div style={{ display: 'flex', flex: 1, minHeight: 0, minWidth: 0 }}>
        ...
        >
          <div className={flush ? 'shell-inner shell-inner--flush' : 'shell-inner'}>
            {hasContentHeader && (
              <header
                className={`shell-header${isMobile ? ' shell-header--mobile-chrome' : ''}`}
                style={flush ? { paddingBottom: 0 } : undefined}
              >
                <div className="shell-header-copy" style={{ minWidth: 0 }}>
                  {title && <h1>{title}</h1>}
                  {subtitle && (
            ...
            {flush ? (
              <div
                className={`shell-flush-body${hasContentHeader ? ' shell-flush-body--pad-header' : ' shell-flush-body--full'}`}
              >
                {children}
              </div>
            ) : (
